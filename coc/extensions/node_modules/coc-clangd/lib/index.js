...
var require_ral = __commonJS((exports2) => {
  ...
  (function(RAL2) {
    function install2(ral) {
      if (ral === void 0) {
        throw new Error(`No runtime abstraction layer provided`);
      }
      _ral = ral;
    }
    RAL2.install = install2;
  })(RAL || (RAL = {}));
...
var require_events = __commonJS((exports2) => {
  ...
  })(Event3 = exports2.Event || (exports2.Event = {}));
  var CallbackList = class {
    ...
    invoke(...args) {
      ...
      const ret2 = [], callbacks = this._callbacks.slice(0), contexts = this._contexts.slice(0);
...
var require_abort_controller = __commonJS((exports2) => {
  __export(exports2, {
...
You may want to install it manually.`;
  ...
  var Install;
  (function(Install2) {
    function install2(release, asset, abort, ui) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        yield fs.promises.chmod(clangdPath, 493);
        yield fs.promises.unlink(zipFile);
        return clangdPath;
      });
    }
    Install2.install = install2;
    function createDirs(ui) {
      return __awaiter(this, void 0, void 0, function* () {
        const install3 = path.join(ui.storagePath, "install");
        const download2 = path.join(ui.storagePath, "download");
        for (const dir of [install3, download2])
          yield fs.promises.mkdir(dir, {recursive: true});
        return {install: install3, download: download2};
      });
    }
    function findExecutable(paths) {
      const filename = os.platform() == "win32" ? "clangd.exe" : "clangd";
      const entry = paths.find((f) => path.posix.basename(f) == filename || path.win32.basename(f) == filename);
      if (entry == null)
...
(function(SwitchSourceHeaderRequest2) {
  SwitchSourceHeaderRequest2.type = new import_vscode_languageserver_protocol.RequestType("textDocument/switchSourceHeader");
})(SwitchSourceHeaderRequest || (SwitchSourceHeaderRequest = {}));
var SymbolInfoRequest;
(function(SymbolInfoRequest2) {
  SymbolInfoRequest2.type = new import_vscode_languageserver_protocol.RequestType("textDocument/symbolInfo");
...
function switchSourceHeader(ctx) {
  return async () => {
    ...
    await import_coc.workspace.jumpTo(dest);
  };
}
...
var SemanticHighlightingFeature = class {
  constructor(client, context) {
    this.scopeTable = [];
    this.bufTokens = {};
    context.subscriptions.push(client.onDidChangeState(({newState}) => {
      if (newState === import_coc3.State.Running) {
        const notification = new import_vscode_languageserver_protocol2.NotificationType("textDocument/semanticHighlighting");
  ...
  async handleNotification(params) {
    const doc = import_coc3.workspace.getDocument(params.textDocument.uri);
    ...
    for (const [line, tokens] of lines.entries()) {
      for (const token of tokens) {
        if (token.kind === "InactiveCode") {
          skipped.push(import_vscode_languageserver_protocol2.Range.create(line, token.character, line, token.character + token.length));
        } else {
          symbols.push({
            id: 0,
            kind: token.kind,
            ranges: [import_vscode_languageserver_protocol2.Range.create(line, token.character, line, token.character + token.length)],
            parentKind: "Unknown",
            storage: "None"
          });
        }
    ...
    await import_coc3.workspace.nvim.call("lsp_cxx_hl#hl#notify_symbols", [doc.bufnr, symbols]);
    if (skipped.length) {
      await import_coc3.workspace.nvim.call("lsp_cxx_hl#hl#notify_skipped", [doc.bufnr, skipped]);
    }
  }
  decodeTokens(tokens) {
    const scopeMask = 65535;
    const lenShift = 16;
    const uint32Size = 4;
    ...
    for (let i = 0, end = buf.length / uint32Size; i < end; i += 2) {
      ...
      retTokens.push({character: start, scopeIndex, length: len, kind: this.decodeKind(kind)});
    }
    return retTokens;
  }
  decodeKind(kind) {
...
var Ctx = class {
  ...
  async startServer(bin, ...features) {
    ...
    const outputChannel = import_coc4.window.createOutputChannel("clangd");
      ...
      middleware: {
        ...
        provideCompletionItem: async (document2, position, context, token, next) => {
          ...
          const items = (Array.isArray(list) ? list : list.items).map((item) => {
            var _a;
            const start = (_a = item.textEdit) == null ? void 0 : _a.range.start;
            if (start) {
              const prefix = document2.getText(import_coc4.Range.create(start, position));
              if (prefix)
    ...
    if (this.config.semanticHighlighting) {
      const lspCxx = await import_coc4.workspace.nvim.call("exists", "g:lsp_cxx_hl_loaded");
      if (lspCxx === 1) {
...
// src/install.ts
var common = __toModule(require_src());
...
var UI = class {
  ...
  set clangdPath(p) {
    this.config.update("path", p, true);
  }
};
...
var ReloadFeature = class {
  ...
  initialize(caps) {
    ...
    const fileWatcher = import_coc6.workspace.createFileSystemWatcher("**/{compile_commands.json,compile_flags.txt}");
  ...
  async reload(url) {
    ...
    if (notification) {
      const msg = `${import_path.basename(url)} has changed, clangd is reloading...`;
      import_coc6.window.showMessage(msg);
    }
    ...
    this.activate();
    if (notification)
      import_coc6.window.showMessage(`clangd has reloaded`);
  }
};
...
async function activate2(context) {
  ...
  const service = import_coc7.services.getService("clangd");
  if (service) {
    import_coc7.window.showMessage(`Looks like you've configured clangd in coc-settings.json, you should remove it to use coc-clangd`, "warning");
    return;
  }
  ...
  try {
    await ctx.startServer(clangdPath, new ReloadFeature(ctx, () => activate2(context)));
  } catch (e) {
    return;
  }
  const fileStatus = new FileStatus();
  context.subscriptions.push(fileStatus, import_coc7.commands.registerCommand("clangd.switchSourceHeader", switchSourceHeader(ctx)), import_coc7.commands.registerCommand("clangd.symbolInfo", symbolInfo(ctx)), ctx.client.onDidChangeState((e) => {
    var _a2;
    if (e.newState === import_coc7.State.Running) {
      (_a2 = ctx.client) == null ? void 0 : _a2.onNotification("textDocument/clangd.fileStatus", (status) => {
        fileStatus.onFileUpdated(status);
      });
    } else if (e.newState === import_coc7.State.Stopped) {
      fileStatus.clear();
    }
  }), import_coc7.workspace.onDidOpenTextDocument(() => {
